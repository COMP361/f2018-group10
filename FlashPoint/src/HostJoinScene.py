...
class HostJoinScene(Scene):
    def __init__(self):
        self.scene = Scene.__init__(self)
    ...
    def _init_btn_join(self, x_pos, y_pos, text):
        box_size = (130, 48)
        ...
        self.scene.sprite_grp.add(self.buttonJoin)

    def _init_btn_host(self, x_pos, y_pos, text):
        box_size = (130, 48)
        self.buttonHost = RectButton(x_pos, y_pos, box_size[0], box_size[1], Color.BLUE, 0,
                                     Text(pygame.font.SysFont('Arial', 20), text, (0, 255, 0, 0)))
        self.scene.sprite_grp.add(self.buttonHost)
